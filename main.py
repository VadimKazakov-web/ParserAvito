...
try:
    handler = logging.handlers.RotatingFileHandler(filename=base_dir / Path("logbook.log"), maxBytes=3000, backupCount=4)
except FileNotFoundError:
    base_dir.mkdir(parents=True, exist_ok=True)
    handler = logging.FileHandler(filename=base_dir / Path("logbook.log"))
