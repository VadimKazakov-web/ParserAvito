...
* Реализована функция обновления программы. После нажатия на кнопку "проверить обновления",
  из хранилища S3 загружается и читается файл version.py. Если номер версии в этом файле отличается от текущей,
...
* После начала использования [Selenium Wire](https://github.com/wkeeling/selenium-wire) в данном проекте,
  иногда стали происходить обрывы соединения. Возможно это связано с некорректным ответом от локального прокси-сервера **Selenium Wire**<br>
  или некорректных cookies.
* Несмотря на использование cookies в запросах, случайного таймаута после открытия каждой web страницы и скроллинга, <br>
  это не предотвращает блокировку ip адреса и предложения решения [капчи](https://ru.wikipedia.org/wiki/Капча) от avito.
  Решение этой проблемы, а также скорость выполнения программы не входит в цели этого проекта.

<h2>Заключение</h2>

Данный проект вырос из консольного скрипта в котором использовались только библиотека **requests** и потоки python.
Он переписывался заново три раза и все еще не лишён недостатков.<br>
Траектория развития с использованием **Selenium** и **tkinter** была выбрана для того,
чтобы их изучить и использовать.
Возможно, альтернативной реализацией могло бы стать приложение на **Django**,
по крайней мере так было бы *красивее*, либо ещё лучше написать расширение для браузера, чтобы использовать уже существующую сессию с avito.
