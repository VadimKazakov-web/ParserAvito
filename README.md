...
* Поток выполнения главной логики программы в классе WorkFlow, выполнен в виде прохода по "длинному" генератору,
  ...
  > 
  >вместо генераторов теперь используются корутины asyncio.
  основная логика программы, проверка флагов, актуализация прогресса - теперь это отдельные задачи asyncio.
...
* После начала использования [Selenium Wire](https://github.com/wkeeling/selenium-wire) в данном проекте,
  иногда стали происходить обрывы соединения. Это происходит за несколько запросов перед блокировкой ip. Возможно это связано с некорректным ответом от локального прокси-сервера **Selenium Wire**<br>
  или некорректными cookies.
